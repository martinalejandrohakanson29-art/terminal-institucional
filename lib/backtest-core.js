...
// RMA / media móvil de Wilder, sembrada con SMA — equivalente a ta.rma() de Pine v5.
// Los valores null/NaN se saltan (arrastran el último valor válido), como fixnan/na en Pine.
function calcRMA(values, period) {
    const n = values.length;
    const out = new Array(n).fill(null);
    let acum = 0, cuenta = 0, prev = null;
    for (let i = 0; i < n; i++) {
        const v = values[i];
        if (v == null || Number.isNaN(v)) { out[i] = prev; continue; }
        if (prev === null) {
            acum += v; cuenta++;
            if (cuenta === period) { prev = acum / period; out[i] = prev; }
        } else {
...
// DI+, DI- y ADX equivalentes a ta.dmi(diLen, adxSmooth) de Pine v5 (RMA de Wilder,
// con suavizado propio para el ADX). Arrays alineados a los closes (null hasta calentar).
function calcDMI(highs, lows, closes, diLen = 14, adxSmooth = 14) {
    const n = closes.length;
    ...
    const tr = new Array(n - 1), plusDM = new Array(n - 1), minusDM = new Array(n - 1);
    for (let i = 1; i < n; i++) {
        const up = highs[i] - highs[i - 1];
        const down = lows[i - 1] - lows[i];
    ...
    const trur     = calcRMA(tr, diLen);
    const plusRma   = calcRMA(plusDM, diLen);
    const minusRma  = calcRMA(minusDM, diLen);
    const dx = new Array(n - 1).fill(null);
    for (let k = 0; k < n - 1; k++) {
        if (trur[k] == null || trur[k] === 0 || plusRma[k] == null || minusRma[k] == null) continue;
        const p = 100 * plusRma[k]  / trur[k];
        const m = 100 * minusRma[k] / trur[k];
        plusDI[k + 1]  = p;
        minusDI[k + 1] = m;
        const suma = p + m;
        dx[k] = 100 * Math.abs(p - m) / (suma === 0 ? 1 : suma);
    }
    const adxRma = calcRMA(dx, adxSmooth);
    for (let k = 0; k < n - 1; k++) if (adxRma[k] != null) adx[k + 1] = adxRma[k];
...
function runBacktest(bars1m, bars5m, bars15m, whalesArr, p, oiArr, lsArr) {
    if (p.strategyType === 'tw_mtf') return require('./tw-backtest').runTWBacktest(bars1m, p, calcCapitalEntrada);
    ...
    // Velas de 1h/4h/1d derivadas de las de 1m: las piden ADX, EMA de Tendencia, RSI y MACD.
    ...
    const bars4h = () => memo('bars|4h', () => agregarVelas1m(bars1m, 14400000));
    const bars1d = () => memo('bars|1d', () => agregarVelas1m(bars1m, 86400000));
    // Resuelve el array de velas para cualquier temporalidad soportada por los filtros HTF.
    const barsPorTf = (tf) => tf === '5m' ? bars5m : tf === '15m' ? bars15m
    ...
    const pbArr1m = {}, pbArr5m = {}, pbArr15m = {}, pbArr1h = {}, pbArr4h = {}, pbArr1d = {};
    for (const { period, tf } of pbEMAConfig) {
        ...
        } else if (tf === '1h' && !pbArr1h[period]) {
            pbArr1h[period] = memo(`emaIdx|1h|${period}`, () => { const b = bars1h(); return indiceHTF(b, calcEMA(b.map(x => parseFloat(x[4])), period)); });
        } else if (tf === '4h' && !pbArr4h[period]) {
            pbArr4h[period] = memo(`emaIdx|4h|${period}`, () => { const b = bars4h(); return indiceHTF(b, calcEMA(b.map(x => parseFloat(x[4])), period)); });
        } else if (tf === '1d' && !pbArr1d[period]) {
            pbArr1d[period] = memo(`emaIdx|1d|${period}`, () => { const b = bars1d(); return indiceHTF(b, calcEMA(b.map(x => parseFloat(x[4])), period)); });
        }
    }
    ...
    if (rsiTf === '1m') {
        ...
        const idx = memo(`rsiIdx|${rsiTf}|${rsiPeriod}`, () => {
            const barsRsi = rsiTf === '1h' ? bars1h() : rsiTf === '4h' ? bars4h() : rsiTf === '1d' ? bars1d() : bars15m;
            const closesRsi = rsiTf === '15m' ? c15m : barsRsi.map(b => parseFloat(b[4]));
            return indiceHTF(barsRsi, calcRSI(closesRsi, rsiPeriod));
        });
        rsiByTs = idx.map; tsRsi = idx.ts;
    }
    ...
    if (macdTf === '1m') {
        ...
        const idx = memo(`macdIdx|${macdTf}|${macdClave}`, () => {
            const barsMacd = macdTf === '1h' ? bars1h() : macdTf === '4h' ? bars4h() : macdTf === '1d' ? bars1d() : bars15m;
            const closesMacd = macdTf === '15m' ? c15m : barsMacd.map(b => parseFloat(b[4]));
            return indiceHTF(barsMacd, macdPares(closesMacd));
        });
        macdByTs = idx.map; tsMACD = idx.ts;
    }
    ...
    // ADX — temporalidad configurable (1m/5m/15m/1h/4h/1d). Las mayores a 15m no se
    // guardan en ningún lado: se derivan agregando las velas 1m del período (agregarVelas1m).
    const adxTf = ['1m', '5m', '15m', '1h', '4h', '1d'].includes(p.adxTf) ? p.adxTf : '15m';
    // Modo del filtro: 'adx' = ADX ≥ umbral (fuerza de tendencia, no direccional);
    // 'dmi' = DI+ ≥ nivel para LONG y DI- ≥ nivel para SHORT (como ta.dmi de TradingView).
    const adxMode   = p.adxMode === 'dmi' ? 'dmi' : 'adx';
    const dmiLen    = p.dmiLength || 14;
    const adxSmooth = p.adxSmoothing || 14;
    let adxDirect = null, adxByTs = null, tsAdx = null;      // modo 'adx'
    let dmiDirect = null, dmiIdx = null;                     // modo 'dmi'
    if (p.useADXFilter) {
        if (adxMode === 'dmi') {
            if (adxTf === '1m') {
                dmiDirect = memo(`dmi|1m|${dmiLen}|${adxSmooth}`, () => calcDMI(h1m(), l1m(), c1m, dmiLen, adxSmooth));
            } else {
                dmiIdx = memo(`dmiIdx|${adxTf}|${dmiLen}|${adxSmooth}`, () => {
                    const b = barsPorTf(adxTf);
                    const d = calcDMI(b.map(x => parseFloat(x[2])), b.map(x => parseFloat(x[3])), b.map(x => parseFloat(x[4])), dmiLen, adxSmooth);
                    const ts = b.map(x => parseInt(x[6])).sort((a, c) => a - c);
                    return { ts, plus: new Map(b.map((x, i) => [parseInt(x[6]), d.plusDI[i]])), minus: new Map(b.map((x, i) => [parseInt(x[6]), d.minusDI[i]])) };
                });
            }
        } else if (adxTf === '1m') {
    ...
    // ── EMAs multi-timeframe estilo TradingView (grupo "TW") ──────────────────────
    // Replica los 3 filtros de EMA de la estrategia de TV que comparten las mismas medias:
    // alineación (fast>mid>slow), cierre respetando la EMA lenta y pullback (cercanía del
    // low/high a cualquiera de las 3 EMAs). Además, alineación de 3 EMAs en 15m y en 1h.
    const useTwEmas = p.useTwEmasFilter === true;
    const twTriple = (tf, fL, mL, sL) => {
        if (tf === '1m') return { direct: [calcEMA(c1m, fL), calcEMA(c1m, mL), calcEMA(c1m, sL)] };
        const b = barsPorTf(tf);
        const cX = b.map(x => parseFloat(x[4]));
        return { idx: [indiceHTF(b, calcEMA(cX, fL)), indiceHTF(b, calcEMA(cX, mL)), indiceHTF(b, calcEMA(cX, sL))] };
    };
    const twValsAt = (t, i, tsClose) => t.direct
        ? t.direct.map(a => a[i] ?? null)
        : t.idx.map(x => lookupHTF(x.ts, x.map, tsClose));
    let twChart = null, tw15 = null, tw1h = null;
    if (useTwEmas) {
        const cf = p.twChartFast || 21, cm = p.twChartMid || 35, cs = p.twChartSlow || 50;
        const ctf = ['1m', '5m', '15m', '1h'].includes(p.twChartTf) ? p.twChartTf : '5m';
        twChart = memo(`twChart|${ctf}|${cf}|${cm}|${cs}`, () => twTriple(ctf, cf, cm, cs));
        if (p.twUse15) tw15 = memo(`tw15|${p.tw15Fast || 21}|${p.tw15Mid || 35}|${p.tw15Slow || 50}`,
            () => twTriple('15m', p.tw15Fast || 21, p.tw15Mid || 35, p.tw15Slow || 50));
        if (p.twUse1h) tw1h = memo(`tw1h|${p.tw1hFast || 21}|${p.tw1hMid || 35}|${p.tw1hSlow || 50}`,
            () => twTriple('1h', p.tw1hFast || 21, p.tw1hMid || 35, p.tw1hSlow || 50));
    }
    ...
    if (useEmaAngFilter) {
        if (emaAngTf === '1m') {
            ...
            const idx = memo(`emaAngIdx|${emaAngTf}|${angClave}`, () => {
                const barsAng = barsPorTf(emaAngTf);
                const cAng = barsAng.map(b => parseFloat(b[4]));
                const hAng = barsAng.map(b => parseFloat(b[2]));
                const lAng = barsAng.map(b => parseFloat(b[3]));
                return indiceHTF(barsAng, calcEMAangSlope(cAng, hAng, lAng, emaAngLen, emaAngAtr, emaAngSlopeBars));
            });
            emaAngByTs = idx.map; tsEmaAng = idx.ts;
        }
    }
    ...
    if (useEmaTrendFilter) {
        if (emaTrendTf === '1m') {
            ...
            const idx = memo(`emaIdx|${emaTrendTf}|${emaTrendLen}`, () => {
                const barsTrend = barsPorTf(emaTrendTf);
    ...
    if (p.useVwapFilter) {
        ...
        } else if (vwapTf_bt === '15m') {
            const idx = memo(`vwapIdx|15m|${vwapSession_bt}`, () => indiceHTF(bars15m, calcVWAP(bars15m, vwapSession_bt)));
            vwapByTs_bt = idx.map; tsVwap_bt = idx.ts;
        } else {
            const idx = memo(`vwapIdx|${vwapTf_bt}|${vwapSession_bt}`, () => {
                const barsVwap = barsPorTf(vwapTf_bt);
                return indiceHTF(barsVwap, calcVWAP(barsVwap, vwapSession_bt));
            });
            vwapByTs_bt = idx.map; tsVwap_bt = idx.ts;
        }
    }
    ...
    const stopEmaArrays = stopEMACfgs.map(({ period, tf }) => {
        if (tf === '5m') {
            ...
            return { arr: null, tsArr: idx.ts, map: idx.map };
        } else if (tf === '1h' || tf === '4h' || tf === '1d') {
            const idx = memo(`emaIdx|${tf}|${period}`, () => {
                const b = barsPorTf(tf);
                return indiceHTF(b, calcEMA(b.map(x => parseFloat(x[4])), period));
            });
            return { arr: null, tsArr: idx.ts, map: idx.map };
        }
    ...
    for (let i = WARMUP; i < HASTA; i++) {
        ...
        const alignVals = pbEMAConfig.map(({ period, tf }) => {
            ...
            if (tf === '1h')  return pbArr1h[period]  ? lookupHTF(pbArr1h[period].ts,  pbArr1h[period].map,  tsClose) : null;
            if (tf === '4h')  return pbArr4h[period]  ? lookupHTF(pbArr4h[period].ts,  pbArr4h[period].map,  tsClose) : null;
            if (tf === '1d')  return pbArr1d[period]  ? lookupHTF(pbArr1d[period].ts,  pbArr1d[period].map,  tsClose) : null;
            return null;
        });
        ...
        for (let j = posiciones.length - 1; j >= 0; j--) {
            ...
            // Si el stop que ejecutó ya había sido movido por la protección escalonada, se
            // etiqueta 'PROT' para distinguirlo del stop inicial en el desglose de salidas.
            if (exitReason === 'SL' && pos.stagedLevel > 0) exitReason = 'PROT';
        ...
        // Protección escalonada de ganancias — al alcanzar el precio el trigger del nivel N a
        // favor, el stop se mueve a entrada ± stopN. Ratchea al CIERRE de la vela y rige desde
        // la próxima (mismo criterio anti look-ahead que el breakeven). Solo con stop 'Porcentaje'.
        if (p.useStagedProtection && p.stopType === 'Porcentaje' && Array.isArray(p.stagedLevels)) {
            for (const pos of posiciones) {
                for (let k = 0; k < p.stagedLevels.length; k++) {
                    const lv = p.stagedLevels[k];
                    if (!lv || !lv.on) continue;
                    const alcanzado = pos.side === 1
                        ? high >= pos.entry * (1 + lv.trig / 100)
                        : low  <= pos.entry * (1 - lv.trig / 100);
                    if (alcanzado) pos.stagedLevel = Math.max(pos.stagedLevel || 0, k + 1);
                }
                if (pos.stagedLevel) {
                    const s = p.stagedLevels[pos.stagedLevel - 1].stop;
                    pos.sl = pos.side === 1 ? pos.entry * (1 + s / 100) : pos.entry * (1 - s / 100);
                }
            }
        }
        ...
        if ((posiciones.length === 0 || p.allowMultipleEntries) && !hayPosicionEnPerdida) {
            ...
            // Filtro por día de la semana. Con useDayFilter se opera solo los días marcados
            // (índice = getUTCDay: 0=Dom..6=Sáb); si está apagado, rige el toggle de fin de semana.
            const dayOk = p.useDayFilter
                ? !!(Array.isArray(p.tradeDays) && p.tradeDays[argDay])
            ...
            if (
                ...
                // ADX/DMI: 'adx' = fuerza de tendencia (>= umbral); 'dmi' = DI+ para long / DI- para short.
                let adxOkLong = true, adxOkShort = true;
                if (p.useADXFilter) {
                    if (adxMode === 'dmi') {
                        const pdi = adxTf === '1m' ? (dmiDirect ? dmiDirect.plusDI[i]  : null) : (dmiIdx ? lookupHTF(dmiIdx.ts, dmiIdx.plus,  tsClose) : null);
                        const mdi = adxTf === '1m' ? (dmiDirect ? dmiDirect.minusDI[i] : null) : (dmiIdx ? lookupHTF(dmiIdx.ts, dmiIdx.minus, tsClose) : null);
                        adxOkLong  = pdi != null && pdi >= (p.diPlusMin  ?? 25);
                        adxOkShort = mdi != null && mdi >= (p.diMinusMin ?? 25);
                    } else {
                        const adxRaw = adxTf === '1m' ? adxDirect[i] : lookupHTF(tsAdx, adxByTs, tsClose);
                        const ok = adxRaw != null && adxRaw >= (p.adxThreshold ?? 25);
                        adxOkLong = ok; adxOkShort = ok;
                    }
                }

                // EMAs multi-timeframe (grupo TW) — alineación / cierre vs EMA lenta / pullback
                let twOkLong = true, twOkShort = true;
                if (useTwEmas) {
                    const [cf, cm, cs] = twValsAt(twChart, i, tsClose);
                    if (cf == null || cm == null || cs == null) { twOkLong = false; twOkShort = false; }
                    else {
                        if (p.twUseAlign !== false) {
                            twOkLong  = twOkLong  && cf > cm && cm > cs;
                            twOkShort = twOkShort && cf < cm && cm < cs;
                        }
                        if (p.twUseSlowClose) {
                            twOkLong  = twOkLong  && close > cs;
                            twOkShort = twOkShort && close < cs;
                        }
                        if (p.twUsePullback) {
                            const maxD = p.twPullbackPerc ?? 0.40;
                            const dl = Math.min(Math.abs(low - cf) / cf, Math.abs(low - cm) / cm, Math.abs(low - cs) / cs) * 100;
                            const dh = Math.min(Math.abs(high - cf) / cf, Math.abs(high - cm) / cm, Math.abs(high - cs) / cs) * 100;
                            twOkLong  = twOkLong  && dl <= maxD;
                            twOkShort = twOkShort && dh <= maxD;
                        }
                    }
                    if (tw15) {
                        const [a, b2, c2] = twValsAt(tw15, i, tsClose);
                        const listo = a != null && b2 != null && c2 != null;
                        twOkLong  = twOkLong  && listo && a > b2 && b2 > c2;
                        twOkShort = twOkShort && listo && a < b2 && b2 < c2;
                    }
                    if (tw1h) {
                        const [a, b2, c2] = twValsAt(tw1h, i, tsClose);
                        const listo = a != null && b2 != null && c2 != null;
                        twOkLong  = twOkLong  && listo && a > b2 && b2 > c2;
                        twOkShort = twOkShort && listo && a < b2 && b2 < c2;
                    }
                ...
                if (p.enableLongs && above && alignLong && (!useRsiFilter || rsiVal >= rsiLongMin) && (!useMacdFilter || macd5 > sig5) && pullOK && deltaOkLong && whaleOkLong && adxOkLong && twOkLong && vwapOkLong && emaAngOkLong && emaTrendOkLong && oiOk && topOkLong && retailOkLong && topSlopeOkLong && cvdOkLong) {
                    ...
                    if (capEntrada <= 0) { /* sin capital disponible, no entrar */ }
                        ...
                        posiciones.push({ side: 1, entry: close, entryBarIdx: i, tp, sl, capitalAtEntry: capEntrada, stagedLevel: 0, oiSlope: oiSlopeVal, topRatio: topRatioVal, globalRatio: globRatioVal });
                    }
                } else if (p.enableShorts && below && alignShort && (!useRsiFilter || rsiVal <= rsiShortMax) && (!useMacdFilter || macd5 < sig5) && pullOK && deltaOkShort && whaleOkShort && adxOkShort && twOkShort && vwapOkShort && emaAngOkShort && emaTrendOkShort && oiOk && topOkShort && retailOkShort && topSlopeOkShort && cvdOkShort) {
                    const capEntrada = calcCapitalEntrada(p, capital, posiciones);
                    if (capEntrada <= 0) { /* sin capital disponible, no entrar */ }
                    else {
                        const tp = close * (1 - p.tpPerc / 100);
...
function normalizarParams(body) {
    const p = {
        ...require('./tw-backtest').normalizeTW(body),
        ...
        stopEMAs:          (() => {
            ...
            return Array.isArray(body.stopEMAs) && body.stopEMAs.length
                ? body.stopEMAs.slice(0,2).map(e => ({ period: Math.max(1, parseInt(e.period)||200), tf: ['1m','5m','15m','1h','4h','1d'].includes(e.tf) ? e.tf : '1m' }))
        ...
        pullbackEMAs:      Array.isArray(body.pullbackEMAs)
                               ? body.pullbackEMAs.map(e => ({
                                   ...
                                   tf:     ['1m','5m','15m','1h','4h','1d'].includes(e.tf) ? e.tf : '1m'
                               }))
                               : [{ period:50,tf:'1m' },{ period:100,tf:'1m' },{ period:200,tf:'1m' },{ period:500,tf:'1m' }],
        useRsiFilter:      body.useRsiFilter !== false,
        rsiTf:             ['1m','5m','15m','1h','4h','1d'].includes(body.rsiTf) ? body.rsiTf : '15m',
        ...
        emaAngTf:          ['1m','5m','15m','1h','4h','1d'].includes(body.emaAngTf) ? body.emaAngTf : '15m',
        ...
        macdTf:            ['1m','5m','15m','1h','4h','1d'].includes(body.macdTf) ? body.macdTf : '5m',
        ...
        adxTf:               ['1m','5m','15m','1h','4h','1d'].includes(body.adxTf) ? body.adxTf : '15m',
        adxThreshold:        parseInt(body.adxThreshold) || 25,
        adxMode:             body.adxMode === 'dmi' ? 'dmi' : 'adx',
        dmiLength:           parseInt(body.dmiLength) || 14,
        adxSmoothing:        parseInt(body.adxSmoothing) || 14,
        diPlusMin:           Number.isFinite(parseFloat(body.diPlusMin))  ? parseFloat(body.diPlusMin)  : 25,
        diMinusMin:          Number.isFinite(parseFloat(body.diMinusMin)) ? parseFloat(body.diMinusMin) : 25,
        useTwEmasFilter:     body.useTwEmasFilter === true,
        twChartTf:           ['1m','5m','15m','1h'].includes(body.twChartTf) ? body.twChartTf : '5m',
        twChartFast:         parseInt(body.twChartFast) || 21,
        twChartMid:          parseInt(body.twChartMid)  || 35,
        twChartSlow:         parseInt(body.twChartSlow) || 50,
        twUseAlign:          body.twUseAlign !== false,
        twUseSlowClose:      body.twUseSlowClose === true,
        twUsePullback:       body.twUsePullback === true,
        twPullbackPerc:      Number.isFinite(parseFloat(body.twPullbackPerc)) ? parseFloat(body.twPullbackPerc) : 0.40,
        twUse15:             body.twUse15 === true,
        tw15Fast:            parseInt(body.tw15Fast) || 21,
        tw15Mid:             parseInt(body.tw15Mid)  || 35,
        tw15Slow:            parseInt(body.tw15Slow) || 50,
        twUse1h:             body.twUse1h === true,
        tw1hFast:            parseInt(body.tw1hFast) || 21,
        tw1hMid:             parseInt(body.tw1hMid)  || 35,
        tw1hSlow:            parseInt(body.tw1hSlow) || 50,
        useStagedProtection: body.useStagedProtection === true,
        stagedLevels:        (() => {
            const def = [[1.0, 0.5], [1.5, 1.0], [2.0, 1.5]];
            const src = Array.isArray(body.stagedLevels) ? body.stagedLevels : [];
            return def.map((d, k) => ({
                on:   src[k] ? src[k].on !== false : true,
                trig: src[k] && Number.isFinite(parseFloat(src[k].trig)) ? parseFloat(src[k].trig) : d[0],
                stop: src[k] && Number.isFinite(parseFloat(src[k].stop)) ? parseFloat(src[k].stop) : d[1],
            }));
        })(),
        useDayFilter:        body.useDayFilter === true,
        tradeDays:           (Array.isArray(body.tradeDays) && body.tradeDays.length === 7)
                                 ? body.tradeDays.map(v => v === true)
                                 : [false, true, true, true, true, true, false],
        useEmaTrendFilter:   body.useEmaTrendFilter === true,
        emaTrendTf:          ['1m','5m','15m','1h','4h','1d'].includes(body.emaTrendTf) ? body.emaTrendTf : '1h',
        emaTrendLen:         parseInt(body.emaTrendLen) || 21,
        useVwapFilter:       body.useVwapFilter === true,
        vwapTf:              ['1m','5m','15m','1h','4h','1d'].includes(body.vwapTf) ? body.vwapTf : '5m',
    ...
    if (p.strategyType === 'tw_mtf') {
        const staged = p.useStagedProtection;
        for (const key of Object.keys(p)) if (key.startsWith('use')) p[key] = false;
        p.useStagedProtection = staged;
        p.allowMultipleEntries = false;
    }
    return p;
}

module.exports = {
    calcEMA, calcRSI, calcMACDArr, calcADX, calcRMA, calcDMI, calcATR, calcEMAangSlope, calcVWAP,
