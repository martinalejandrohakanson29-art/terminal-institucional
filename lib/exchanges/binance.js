...
const PRICE_DECIMALS = (PRICE_STEP.toString().split('.')[1] || '').length;
const BASE_REAL = 'https://fapi.binance.com';
const BASE_DEMO = 'https://testnet.binancefuture.com';

// fapi (USDⓈ-M Futures), NO api.binance.com (spot): la cuenta opera y su TP/SL/breakeven
// se evalúan contra el precio de futuros (ver whaleFeeds PERP, openInterest y ejecución más
// abajo, todos ya en fapi) — con spot, la señal y los niveles vivían en un mercado distinto
// al que realmente tradea el bot. El basis entre ambos ronda unos dólares pero alcanza para
// que el backtest muestre un TP tocado que en vivo nunca se tocó (ver caso real: entrada
// #366 del admin, 2026-08-19). Mismo shape de array que el endpoint de spot.
...
// Precio actual vía REST público (sin firma) — fallback para cuando el WS de precio no
// tiene dato en caché (ver caso real: fstream.binance.com conecta pero no entrega ticks
// desde algunos hosting; el REST sí responde con el mismo endpoint que usa getBalance).
async function getPrice(ctx) {
    const r = await fetch(`${ctx.base}/fapi/v1/ticker/price?symbol=${SYMBOL}`);
    const j = await r.json();
    const p = parseFloat(j?.price);
    if (!(p > 0)) throw new Error((j && j.msg) || 'precio no disponible');
    return p;
}
...
// Devuelve STRING, no number: Math.round(precio/PRICE_STEP)*PRICE_STEP arrastra basura de
// punto flotante (ej. 63543.140 -> 63543.100000000006) que Binance rechazaba con -1111
// "Precision is over the maximum defined for this asset" — pasó en vivo el 2026-08-14 y dejó
// una posición sin SL en el exchange durante 3 días. toFixed fuerza la precisión real del
// tick size y corta ese resto.
function redondearPrecio(precio) {
    return (Math.round(precio / PRICE_STEP) * PRICE_STEP).toFixed(PRICE_DECIMALS);
}

// Consulta el fill real de una orden ya colocada. Usado como reintento cuando la respuesta
// de placeEntryOrder no trae avgPrice (Binance a veces confirma la orden antes de que el
// match termine de propagarse) — sin esto, precio_entrada y por ende TP/SL/breakeven quedaban
// calculados sobre el precio de señal en vez del costo real (ver caso real: entrada #366 del
// admin, 2026-08-19).
async function getOrderFill(ctx, orderId) {
    const ts  = Date.now();
    ...
    const r    = await fetch(url, { headers: { 'X-MBX-APIKEY': ctx.apiKey } });
    const body = await r.json();
    const p = parseFloat(body?.avgPrice);
    return p > 0 ? p : null;
}
