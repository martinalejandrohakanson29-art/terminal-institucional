...
// A diferencia de Binance, BingX devuelve positionAmt SIN signo (siempre positivo):
// la dirección viene aparte en positionSide ('LONG'/'SHORT'). Hay que aplicar el signo
// acá para que el resultado sea comparable con el libro interno (positivo=long,
// negativo=short) — confirmado en logs reales: una posición SHORT abierta reportaba
// positionAmt positivo, generando falsas "discrepancias" en cada reconciliación.
async function getPositionAmt(ctx) {
    ...
    return rows
        ...
        .reduce((sum, p) => sum + (p.positionSide === 'SHORT' ? -1 : 1) * parseFloat(p.positionAmt), 0);
}

// Ver la nota equivalente en binance.js: Math.round(precio/PRICE_STEP)*PRICE_STEP arrastra
// basura de punto flotante (ej. 63543.140 -> 63543.100000000006) que el exchange puede
// rechazar por precisión — toFixed corta ese resto y devuelve el tick size real.
...
// Ver la nota equivalente en binance.js: reintento cuando placeEntryOrder no trae avgPrice
// en la respuesta inmediata de la orden MARKET.
async function getOrderFill(ctx, orderId) {
    const { ok, body } = await request(ctx, 'GET', '/openApi/swap/v2/trade/order', { symbol: SYMBOL, orderId });
    if (!ok) return null;
    const o = body?.data?.order;
    const p = o ? parseFloat(o.avgPrice) : NaN;
    return p > 0 ? p : null;
}
...
// En modo One-way, BingX EXIGE side:'BOTH' y rechaza LONG/SHORT (109400 "In the One-way
// mode, the 'Side' field can only be set to BOTH" — confirmado contra la API en vivo el
// 2026-07-26; la doc pública describe el comportamiento de Hedge Mode, no el de One-way).
async function setLeverage(ctx, leverage) {
    const lev = Math.round(leverage);
    const r = await request(ctx, 'POST', '/openApi/swap/v2/trade/leverage', {
        symbol: SYMBOL, side: 'BOTH', leverage: lev,
    });
    if (r.ok) console.log(`[BingX u${ctx.uid}] Leverage ${lev}x ✅`);
    else console.warn(`[BingX u${ctx.uid}] ⚠️ No se pudo fijar leverage: ${r.body?.code} ${r.body?.msg}`);
    return r.ok;
}
