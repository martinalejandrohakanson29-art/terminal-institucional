...
async function placeEntryOrder(ctx, { lado, qty }) {
    const r = await ejecutarOrden(ctx, {
        symbol: SYMBOL, type: 'MARKET', side: lado === 'long' ? 'BUY' : 'SELL', quantity: qty,
    }, 'ENTRADA');
...
async function placeCloseOrder(ctx, { lado, qty }) {
    return ejecutarOrden(ctx, {
        symbol: SYMBOL, type: 'MARKET', side: lado === 'long' ? 'SELL' : 'BUY', quantity: qty, reduceOnly: 'true',
    }, 'CIERRE');
}

async function placeStopOrder(ctx, { lado, qty, stopPrice, tipo }) {
    const r = await ejecutarOrden(ctx, {
        symbol: SYMBOL, type: tipo, side: lado === 'long' ? 'SELL' : 'BUY', quantity: qty,
...
async function setLeverage(ctx, leverage) {
    const r = await request(ctx, 'POST', '/openApi/swap/v2/trade/leverage', {
        symbol: SYMBOL, side: 'BOTH', leverage: Math.round(leverage),
    });
    if (r.ok) console.log(`[BingX u${ctx.uid}] Leverage ${Math.round(leverage)}x ✅`);
