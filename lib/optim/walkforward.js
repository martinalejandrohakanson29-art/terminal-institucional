 ...
 *
 * Los trials se despachan EN LOTES del tamaño del pool: se proponen N combinaciones con el
 * mismo estado del modelo, se evalúan en paralelo, y se registran los resultados en orden de
 * índice antes de proponer el lote siguiente.
 *
 * El orden de registro es lo que hace reproducible al estudio. Despachando de forma continua
 * —repone un trial apenas se libera un worker— se usa mejor la CPU, pero el modelo del TPE
 * recibe los resultados en el orden en que terminan, que depende de cuánto tardó cada
 * backtest. Dos corridas con la misma semilla proponían combinaciones distintas a partir del
 * primer desempate. Con lotes se pierde algo de adaptación (el modelo se actualiza cada N
 * trials en vez de cada uno, con N = 3 típicamente) y algo de CPU en los rezagados de cada
 * lote, a cambio de que --semilla signifique de verdad "misma entrada, mismo resultado".
 */
async function optimizarVentana({ pool, espacio, ventana, trials, topN, semilla, alRegistrar, usarPruning = true }) {
    ...
    while (lanzados < trials) {
        const lote = [];
        for (let i = 0; i < Math.min(pool.n, trials - lanzados); i++) {
            lote.push({ nro: ++lanzados, cfg: muestreador.sugerir() });
        }

        if (usarPruning) {
            const parciales = await Promise.all(lote.map(t => pool.evaluar(t.cfg, FRACCION_PRUNING)));
            // Decidir y registrar en orden de índice, no de llegada.
            lote.forEach((t, i) => {
                t.parcial = parciales[i];
                t.podado = podador.debePodar(t.parcial.score);
                podador.registrar(t.parcial.score);
            });
        }

        const sobreviven = lote.filter(t => !t.podado);
        const completos = await Promise.all(sobreviven.map(t => pool.evaluar(t.cfg, 1)));
        sobreviven.forEach((t, i) => { t.completo = completos[i]; });
        ...
        for (const t of lote) {
            if (t.podado) {
                ...
                if (alRegistrar) alRegistrar({ nroTrial: t.nro, cfg: t.cfg, podado: true, metricas: t.parcial.metricas, score: t.parcial.score });
                continue;
            }
            muestreador.registrar(t.cfg, t.completo.score);
            completados++;
            if (alRegistrar) alRegistrar({ nroTrial: t.nro, cfg: t.cfg, podado: false, metricas: t.completo.metricas, score: t.completo.score });
        }
    }
