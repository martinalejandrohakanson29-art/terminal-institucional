...
function normalizeTW(body) {
    const levelDefaults = [[1, .5], [1.5, 1], [2, 1.5]];
    const sourceLevels = Array.isArray(body.stagedLevels) ? body.stagedLevels : [];
    return {
        ...
        twRiskMode: body.twRiskMode === 'manual' ? 'manual' : 'structural',
        twRewardRisk: bounded(body.twRewardRisk, 2, .1, 20),
        twTakeProfitPerc: bounded(body.twTakeProfitPerc, 1, .001, 100),
        twStopLossPerc: bounded(body.twStopLossPerc, 1, .001, 100),
        ...
        stagedLevels: levelDefaults.map((defaults, index) => {
            const level = sourceLevels[index] || {};
            return { on: level.on !== false, trig: bounded(level.trig, defaults[0], .001, 100),
                stop: bounded(level.stop, defaults[1], -100, 100) };
        })
    };
}
function levels(event, entry, input) {
    ...
    const sl = p.twRiskMode === 'manual'
        ? entry * (1 - side * p.twStopLossPerc / 100)
        : event.stopReference * (1 - side * p.twStopBufferPerc / 100);
    const risk = side * (entry - sl), riskPerc = risk / entry * 100;
    const tp = p.twRiskMode === 'manual'
        ? entry * (1 + side * p.twTakeProfitPerc / 100)
        : entry + side * risk * p.twRewardRisk;
    const leverage = p.palancaActivo ? Number(p.palancaValor) : 1;
    const structuralRiskInvalid = p.twRiskMode === 'structural' &&
        (riskPerc < p.twMinStopPerc || riskPerc > p.twMaxStopPerc);
    if (!(entry > 0) || !Number.isFinite(entry) || !Number.isFinite(risk) || risk <= 0 || sl <= 0 || tp <= 0 ||
        structuralRiskInvalid || (leverage > 1 && riskPerc >= 100 / leverage)) return null;
...
function runTWBacktest(inputRows, inputParams, sizePosition) {
    ...
    if (p.twRiskMode === 'structural' && p.twMinStopPerc > p.twMaxStopPerc) throw new Error('TW: el stop mínimo no puede superar al máximo.');
    ...
    for (let segmentIndex = 0; segmentIndex < segments.length; segmentIndex++) {
        ...
        function close(rawPrice, ts, reason) {
            ...
            trades.push({ ...pos, type: pos.side === 1 ? 'Long' : 'Short', exitTs: ts, exitPrice: money.fill,
                ...
                realizedR: money.pnl / (pos.qty * Math.abs(pos.entryPrice - pos.initialSl)) });
            position = null;
        }
        for (let i = 0; i < part.length; i++) {
            ...
            if (position) rejected.occupied += eligible.length;
                ...
                if (new Set(eligible.map(e => e.side)).size > 1) rejected.conflict += eligible.length;
                    ...
                    const riskLevels = levels(e, entryPrice, p);
                    const sl = riskLevels?.sl, tp = riskLevels?.tp;
                    const liquidation = entryPrice * (1 - side / leverage);
                    if (!riskLevels || (leverage > 1 && side * (sl - liquidation) <= 0)) rejected.risk++;
                    else {
                        const margin = sizePosition({ ...p, initialCapital: capitalInitial }, capital, []);
                        if (!Number.isFinite(margin) || margin <= 0) rejected.capital++;
                        else position = { side, entryTs: ts, entryPrice, sl, initialSl: sl, tp, margin, qty: margin * leverage / entryPrice, stagedLevel: 0,
            ...
            if (position) {
                ...
                else if (long ? open <= pos.sl : open >= pos.sl) close(open, ts, pos.stagedLevel ? 'PROT' : 'SL');
                else if (long ? open >= pos.tp : open <= pos.tp) close(pos.tp, ts, 'TP');
                else if (long ? low <= pos.sl : high >= pos.sl) close(pos.sl, ts + MINUTE, pos.stagedLevel ? 'PROT' : 'SL');
            ...
            // Igual que la protección clásica: se confirma con el extremo de la vela cerrada
            // y el nuevo stop empieza a regir recién en la vela siguiente.
            if (position && p.useStagedProtection) {
                for (let k = 0; k < p.stagedLevels.length; k++) {
                    const lv = p.stagedLevels[k];
                    if (!lv.on) continue;
                    const reached = position.side === 1
                        ? high >= position.entryPrice * (1 + lv.trig / 100)
                        : low <= position.entryPrice * (1 - lv.trig / 100);
                    if (reached) position.stagedLevel = Math.max(position.stagedLevel, k + 1);
                }
                if (position.stagedLevel) {
                    const offset = p.stagedLevels[position.stagedLevel - 1].stop;
                    const candidate = position.entryPrice * (1 + position.side * offset / 100);
                    position.sl = position.side === 1 ? Math.max(position.sl, candidate) : Math.min(position.sl, candidate);
                }
            }
...
module.exports = { normalizeTW, levels, warmupMs, aggregateComplete, summarize, runTWBacktest };
